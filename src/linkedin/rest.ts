...
/**
 * Generic LinkedIn GET with auth + error handling
 */
export async function httpGet(
  ...
  if (Date.now() >= EXPIRES_AT) {
    throw new Error("Access token expired. Re-login.");
  }

  const r = await fetch(url, { headers: { ...H(), ...extra } });
  if (!r.ok) {
    throw new Error(`LinkedIn GET ${r.status}: ${await r.text()}`);
  }
  return (await r.json()) as any;
}

/**
 * Exchange OAuth2 code for a bearer token
 */
export async function exchangeCodeForToken(code: string) {
  ...
  EXPIRES_AT = Date.now() + j.expires_in * 1000 - 60_000; // renew 1 min early
  return ACCESS_TOKEN;
}

/**
 * Concrete LinkedIn client using REST API
 */
export class RestLinkedIn implements LinkedInClient {
  async getOrgUrnFromVanity(vanity: string): Promise<string> {
    const url = `https://api.linkedin.com/rest/organizations?q=vanityName&vanityName=${enc(
      vanity
    )}`;
  ...
  async getOrgPosts(orgUrn: string, count = 50): Promise<string[]> {
    const url = `https://api.linkedin.com/rest/posts?q=author&author=${enc(
      orgUrn
  ...
  async getReactors(postUrn: string, count = 100): Promise<string[]> {
    const url = `https://api.linkedin.com/rest/reactions/(entity:${enc(
      postUrn
    )})?q=entity&count=${count}`;
    const j: any = await httpGet(url);
    const els = j?.elements || [];
    return els
      .map(
        (r: any) =>
          String(r.id || "").match(/urn:li:person:[A-Za-z0-9_-]+/)?.[0] || null
      )
      .filter(Boolean) as string[];
  }

  async getCommenters(postUrn: string, count = 100): Promise<string[]> {
    const url = `https://api.linkedin.com/rest/socialActions/${enc(
      postUrn
    ...
    return els
      .map((c: any) => c.actor)
      .filter((a: string) => a?.startsWith("urn:li:person:"));
  }
}
